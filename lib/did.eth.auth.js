...
module.exports = class EthDidClient {
  ...
  async init(){
    const endPoint = this.cfg.network;
    const abiPath = this.cfg.regABI;
    const provider = this.getProvdier(endPoint);
    this.web3 = new Web3(provider);
    const netID = await this.web3.eth.net.getId();
    const srABI = require(abiPath);
    const srInfo = srABI.networks[netID];
    this.account = this.cfg.account;
    this.password = this.cfg.password;
    this.didReg = await new this.web3.eth.Contract(srABI.abi, srInfo.address);
  }
  ...
  async createDocument(){
    if(this.account == null){
      return this._returnMsg(-1,'account is required!');
    }
    try{
      const from = this.account
      await this.didReg.methods.create().send({from: from, gas: gas, gasPrice: 1});
  ...
  creatPairKey(type){
      if(type == 'EcdsaSecp256k1RecoveryMethod2020'){
        const result = this.web3.eth.accounts.create();
        return{
  ...
  * @return signature: hex string ex. 0x (string) , VRS: {v:int, r:string, s:stirng}* */ 
  sign(data, type, privateKey){
    if(type == 'EcdsaSecp256k1RecoveryMethod2020'){
      const signObj = this.web3.eth.accounts.sign(data,privateKey);
      const vrsObj = {
  ...
  async setController(did, delegate){
    ...
    try{
      ...
      await this.didReg.methods.setController(did, delegate).send({from: from, gas: gas, gasPrice: 1});
  ...
  async setControllerBySigner(did, delegate, sig){
    ...
    try{
      const from = this.account
      await this.didReg.methods.setControllerBySign(did, delegate, sig.v, sig.r, sig.s).send({from: from, gas: gas, gasPrice: 1});
   ...
   * @return statusCode: 
   *           -1: Login is required!    -2: error.msg    1: success  */  
  async updatePrivacy(did, scvID, hashVal){
    if(this.account == null){
      return this._returnMsg(-1,'account is required!');
    }
    try{
      const from = this.account;
      await this.didReg.methods.updatePrivacy(did, scvID, hashVal).send({from: from, gas: gas, gasPrice: 1});
      return this._returnMsg(1,'Success update privacy');
    }catch(e){
      return this._returnMsg(-2, e.message);
    }
  } 
  ...
  async updatePrivacy(did, scvID, hashVal){
    ...
    try{
      const from = this.account;
      await this.didReg.methods.updatePrivacy(did, scvID, hashVal,  sig.v, sig.r, sig.s).send({from: from, gas: gas, gasPrice: 1});
      return this._returnMsg(1,'Success update privacy');
    }catch(e){
      return this._returnMsg(-2, e.message);
    }
  ...
  *           -3: Not vaild key type     1: success */  
  async addPubKeyBySigner(did, type, publicKey, controller, sig){
    ...
    try{
      ...
      if(type == 'EcdsaSecp256k1RecoveryMethod2020'){
        await this.didReg.methods.addAddrKeyBySign(did, publicKey, controller, sig.v, sig.r, sig.s).send({from: from,gas: gas, gasPrice: 1});
      }else if(type == 'EcdsaSecp256k1VerificationKey2019'){
        await this.didReg.methods.addPubKeyBySign(did, publicKey, controller, sig.v, sig.r, sig.s ).send({from: from, gas: gas, gasPrice: 1});
      }else{
        return this._returnMsg(-3, 'Not valid type!');
      }
  ...
  *           -1: Login is required!  -2: error.msg  1: success  */    
  async addService(did, id, type, endpoint){
    ...
    try{
      ...
      await this.didReg.methods.addService(did, id, type, endpoint).send({from: from, gas: gas, gasPrice: 1});
  ...
  *           -1: Login is required!  -2: error.msg  1: success  */      
  async addServiceBySinger(did, id, type, endpoint, sig){
    ...
    try{
      ...
      await this.didReg.methods.addServiceBySign(did, id, type, endpoint, sig.v, sig.r, sig.s).send({from: from, gas: gas, gasPrice: 1});
  ...
  async revokePubKey(did, pubKeyId){
    ...
    try{
      const from = this.account
      await this.didReg.methods.disableKey(did, pubKeyId).send({from: from, gas: gas, gasPrice: 1});
  ...
  async revokePubKeyBySinger(did, pubKeyId, sig){
    ...
    try{
      const from = this.account
      await this.didReg.methods.disableKey(did, pubKeyId, sig.v, sig.r, sig.s).send({from: from, gas: gas, gasPrice: 1});
  ...
  async revokeService(did, scvId){
    ...
    try{
      const from = this.account
      await this.didReg.methods.disableService(did, scvId).send({from: from, gas: gas, gasPrice: 1});
  ...
  async revokeServiceBySinger(did, scvId, sig){
    ...
    try{
      const from = this.account
      await this.didReg.methods.disableService(did, scvId, sig.v, sig.r, sig.s).send({from: from, gas: gas, gasPrice: 1});
  ...
  */
  _isValid_Secpk1_Recovery2020(signature, data, pubKeyAddr){
    const signerAddress = this.web3.eth.accounts.recover(data, signature);
